...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="mail_registrated_subject">
		<xsl:text>Регистрация на сайте </xsl:text><xsl:value-of select="domain"/>
	</xsl:template>

	<xsl:template match="mail_registrated">
		<xsl:variable name="curr" select="document(concat('uobject://',user_id))/udata"/>
		<p>
		...
		<p>
			...
			<xsl:value-of select="$curr//property[@name='e-mail']/value" /><br />
	...
	<xsl:variable name="curr" select="document(concat('uobject://',user_id))/udata"/>
		<p>
			<xsl:text>Зарегистрировался новый пользователь "</xsl:text>
			<xsl:value-of select="$curr//property[@name='e-mail']/value" />
			<xsl:text>".</xsl:text>
		</p>
	</xsl:template>
