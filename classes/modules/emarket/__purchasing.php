		...
		*/
		public function getBasketOrder($useDummyOrder = true) {
			...
			if($sel->length()) {
				...
				// подбираем заказ, который хочет редактировать менеджер от лица пользователя
				if(getSession('fake-user')) {
					foreach ($result as $order) {
						if($order->status_id == order::getStatusByCode('editing')) {
		...
		public function purchase($template = 'default') {
			...
			else if (!def_module::isXSLTResultMode()) {
